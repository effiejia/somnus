...
import ShareSendButtons from "@/components/ShareSendButtons";
import { generateShareToken, removeShareToken } from "@/lib/api/sharing";
...
export default function ShareModal({ dream, onClose, onTokenChange }: ShareModalProps) {
	...
	const smsUrl = shareUrl
		? `sms:?body=${encodeURIComponent(`Check out this dream on Somnus: ${shareUrl}`)}`
		: "#";
	const mailUrl = shareUrl
		? `mailto:?subject=${encodeURIComponent(dream.title || "A dream on Somnus")}&body=${encodeURIComponent(`I recorded a dream on Somnus and wanted to share it with you:\n\n${shareUrl}`)}`
		: "#";
				...
				<div className="px-5 pb-5 space-y-4">
					...
					<div className="flex items-center gap-2 bg-[#1a1a1a] rounded-xl px-3 py-2.5">
						...
						<button
							...
							className="shrink-0 text-xs font-medium px-3 py-1 rounded-full bg-[#2a2a2a] hover:bg-[#333] text-[#ededed] transition-colors disabled:opacity-40"
						>
							{copied ? "Copied!" : "Copy"}
						</button>
					</div>
